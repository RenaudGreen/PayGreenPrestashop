...
class Paygreen extends PaymentModule
{
    ...
    public function install()
    {
        ...
        try {
            ...
            if (!PaygreenDatabase::createTable()) {
                return false;
            }
        } catch (Exception $ex) {
    ...
    */
    ...
    public function validateWebIframePayment($a_data, $isCallback)
    {
        ...
        if ($client->data->result->status == PaygreenClient::STATUS_REFUSED) {
            $status = Configuration::get('PS_OS_ERROR');
            ...
            $status = Configuration::get(self::_CONFIG_ORDER_AUTH);
            $f_amount = 0;
        } elseif ($client->data->result->status == PaygreenClient::STATUS_SUCCESSED) {
            $status = Configuration::get('PS_OS_PAYMENT');
        } else {
            if (!$isCallback) {
                $this->redirectToConfirmationPage();
        ...
        if ((int)($client->data->testMode) == 1 && $status == Configuration::get('PS_OS_PAYMENT')) { //_CONFIG_CANCEL_ACTION
            $status = Configuration::get(self::_CONFIG_ORDER_TEST);
            $f_amount = 0;
        }
        $a_vars = (array)$client->data->result;
        $a_vars['date'] = time();
        $a_vars['transaction_id'] = $client->data->orderId;
        $a_vars['mode'] = $client->data->type;
        $a_vars['amount'] = $client->data->amount;
        $a_vars['currency'] = $client->data->currency;
        $a_vars['by'] = 'webPayment';

        $n_order_id = (int)Order::getOrderByCartId($o_cart->id);
        if (!$this->isPaygreenSamePID($client->data->metadata->cart_id, Tools::substr($client->data->id, 2))) {
            //TODO when card_id have 2 paygreen_id print message on prestashop
            //throw new Exception();
        }
    ...
    */
    ...
    public function getAllPromoCode()
    {
        ...
        $n_array = array();
        $n_array["none"] = $this->l('No Reduction');
    ...
    public function paygreenShippedTransaction($id_order)
    {
        ...
        if (isset($shippedStatus->success)) {
            if ($shippedStatus->success &&
                ($shippedStatus->data->result->status == PaygreenClient::STATUS_SUCCESSED
                    || $shippedStatus->data->result->status == PaygreenClient::STATUS_WAITING_EXEC
                )
            ) {
                $history->changeIdOrderState(Configuration::get('PS_OS_PAYMENT'), (int)($id_order));
                $history->add();
            } else {
                $error = $this->l('la commande n\'a pas pu être expedié.Veuillez réessayer.');
